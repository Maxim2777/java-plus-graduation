...
Проект реализован на основе микросервисной архитектуры. Выделены следующие микросервисы:
...
Взаимодействие между микросервисами осуществляется через **OpenFeign**, сервисы обнаруживаются через **Eureka**, а конфигурация централизованно управляется через **Spring Cloud Config Server**.

---

## 2. Конфигурации

- Конфигурации для каждого микросервиса хранятся в отдельном репозитории **config-repo**.
- В каждом `application.yml` определён импорт конфигации из Config Server:
...
## Внутренние API (межсервисное взаимодействие)

Микросервисы используют следующие внутренние эндпоинты для обмена данными:

- `GET /internal/events/{id}` — получить подробную информацию о событии;
- `GET /internal/events/{eventId}/requests` — получить заявки к событию;
- `POST /internal/requests/batch` — массовое обновление статусов заявок;
- `GET /admin/users/{userId}` — получение пользователя по ID;
- `GET /admin/users` — фильтрация по списку ID.

Все вызовы осуществляются через **Feign-клиенты**, а при недоступности подключаются **Fallback-классы** с дефолтными значениями (например, `null` или пустой список).